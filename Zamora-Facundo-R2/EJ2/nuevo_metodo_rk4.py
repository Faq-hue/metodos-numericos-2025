...
def main():
    ...
    # Método de RK4
    k1 = f(x[0], y[0])
    k2 = f(x[0] + (1/3) * h, y[0] + (1/3) * k1 * h)
    k3 = f(x[0] + (2/3) * h, y[0] - (1/3) * k1 * h + h * k2)
    k4 = f(x[0] + h, y[0] + k1 * h- h*k2+ h*k3)
    y[1] = y[0] + (h / 8) * (k1 + 3*k2 + 3*k3 + k4)
    x[1] = x[0] + 1*h
    ...
    with open("datos_nuevo_metodo_rk4.dat", "w") as f_out:
        for j in range(N+1):
            err = abs(y_exacta(x[j]) - y[j])
            f_out.write(f"{x[j]:.10f}\t{y[j]:.10f}\t{y_exacta(x[j]):.10f}\t{err:.10e}\n")

    print("\nResultados guardados en 'datos_nuevo_metodo_rk4.dat'.")
